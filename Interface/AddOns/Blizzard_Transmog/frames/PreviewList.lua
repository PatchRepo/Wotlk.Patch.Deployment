...
local function button_OnClick(self, button)
	local mainTransmogFrame = self:GetParent():GetParent()
	local onItemClick = mainTransmogFrame.onItemClick
	mainTransmogFrame.selectedItemId = self:GetParent().itemId
	mainTransmogFrame.selectedItemIndex = self:GetParent().itemIndex
	if mainTransmogFrame.selectedItemId ~= nil then
		for _, dr in ipairs(mainTransmogFrame.dressingRooms) do
			if dr.itemId == mainTransmogFrame.selectedItemId then
...
local function PreviewList_SetupModel(self, width, height, x, y, z, facing, sequence)
	-- @HelloKitty: Because it's possible due to appearance tracking for items to be empty this assert is disabled
...
local function handleItemQuerySuccess(dr, itemId)
	dr.queriedLabel:Hide()
	dr:Reset()
	dr:Undress()
...
local function queryItemHandler(functable, itemId, success)
	...
	if dr.itemId == itemId then
		...
		dr.isQuerying = false
		if success then
			handleItemQuerySuccess(dr, itemId);
		else
...
local function PreviewList_Update(self)
	...
	for i, dr in ipairs(self.dressingRooms) do
		...
		if itemId == nil then
			...
			-- @HelloKitty: Because queries actually get send in delayed small batches we need to send an immediate one
