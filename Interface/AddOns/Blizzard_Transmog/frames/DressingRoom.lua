...
function ns.CreateDressingRoom(name, parent)
	...
	function frame:ApplyTransmog(self)
		ClearCurrentTransmogApplyData();
		for _, slot in pairs(mainTransmogFrame.slots) do
			if (slot.slotName ~= nil) then
				
				-- @HelloKitty: This change (and the removed checks above) allow you to clear currently transmogged slots
				if (slot.itemId == nil) then
					slot.itemId = 0;
				end
				
				local characterSlotName = slot.slotName
				...
				if characterSlotName == "Main Hand" then characterSlotName = "MainHand" end
				if characterSlotName == "Off-hand" then characterSlotName = "SecondaryHand" end
				if characterSlotName == "Ranged" then characterSlotName = "Ranged" end
				if characterSlotName == "Back" then characterSlotName = "Back" end
				local slotId = GetInventorySlotInfo(characterSlotName.."Slot");

				-- @HelloKitty: We have slot id, now we can
				AddTransmogApplyData(slotId, slot.itemId);
			end
		end
