...
function FeedbackUI_SetupWelcome (objectName, allowSurvey)
	...
	if ( not objectName ) then
		...
		-- @HelloKitty: I don't want surveys EVER
		FeedbackUI_DisableSurvey(nil);
...
function FeedbackUI_SetupItem (link)  
	...
	function FeedbackUI_SetupSpell (link) 
		...
		function FeedbackUI_SetupTalent(link)
			...
			function FeedbackUI_SpellButton_OnModifiedClick(self, button)
				local targetID;
				if (IsModifiedClick("GENERATEFEEDBACK")) then
					local id = SpellBook_GetSpellID(self:GetID());
					FeedbackUI_SetupSpell(GetSpellLink(id, SpellBookFrame.bookType));
				end
			end
